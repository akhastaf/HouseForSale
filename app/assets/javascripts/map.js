...
function gmap_show(map) {
  if ((map.lat == null) || (map.lng == null) ) {    // validation check if coordinates are there
    return 0;
  }
  ...
  handler.buildMap({ provider: {}, internal: {id: 'map'}}, function(){
    markers = handler.addMarkers([    // put marker method
      {
        "lat": map.lat,    // coordinates from parameter company
        "lng": map.lng,
...
function gmap_form(map) {
  handler = Gmaps.build('Google');    // map init
  handler.buildMap({ provider: {}, internal: {id: 'map'}}, function(){
    if (map && map.lat && map.lng) {    // statement check - new or edit view
